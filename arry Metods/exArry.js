const data = [
  ...
  const johnDoe = data.filter(person => person.name === "John Doe")[0].friends;
  ...
  let yorkers = data.filter(person=> person.address.city==="New York")
  ;
  console.log(yorkers)
...
//   let olders = data.filter(person =>)
  ...
  const address = data.map((person)=>person.address)
  console.log(address);
